...
// T-Deck launcher lock/wake gating (defined in TFTView_320x240.cpp):
//  tdeck_input_gated -> swallow touch/keyboard/trackball-roll while the screen is dark
//                       so that ONLY a trackball double-click can wake it.
//  tdeck_hold_dark   -> manual lock: snap the backlight to black immediately and hold it
//                       there (vs. the gentle fade used for an idle timeout).
//  tdeck_prog_mode   -> BT programming mode is showing: never dim/gate/lock, or the screen
//                       would go dark with touch swallowed and trap the user (the only wake,
...
{
    // handle display timeout (never while BT programming mode is up — dimming there dead-ends
    // the user: touch gets gated and the trackball-double-click wake isn't active on that screen)
    if (!tdeck_prog_mode &&
        ((screenTimeout > 0 && lv_display_get_inactive_time(NULL) > screenTimeout) || powerSaving ||
         (DisplayDriver::view->isScreenLocked()) || tdeck_hold_dark)) {
        ...
        if (DisplayDriver::view->getInputDriver()->hasPointerDevice() || hasTouch() ||
            ...
            if (hasLight()) {
                // T-Deck fix: dim the backlight down to off on inactivity, but NEVER enter
                // ESP32 light-sleep. On USB power, USB-serial traffic repeatedly kicks the chip
                // out of light-sleep in a half-woken state where the backlight never comes back
                // and the device looks hung / can't be woken (plugged-in "won't wake" bug).
                // Keeping the CPU running means the wake path below (the outer else-if) restores
...
{
    // While the screen is dark/locked, swallow touch so it neither wakes the screen nor
    // presses hidden buttons. Only a trackball double-click wakes (see EncoderInputDriver).
