...
void MapPanel::screenToGeo(int16_t x, int16_t y, float &lat, float &lon)
{
    // scrolled sits at the panel center; move a copy so its pixel position lands on (x,y),
    // then read back the recalculated lat/lon (GeoPoint::move does xPos -= scrollX).
    GeoPoint p = scrolled;
    p.move((int16_t)(widthPixel / 2 - x), (int16_t)(heightPixel / 2 - y));
    lat = p.latitude;
    lon = p.longitude;
}
...
// Free the cached tiles now (reclaims the bulk of a map's RAM) and arm a full
// rebuild for when the map is next drawn. Setting needsRedraw makes the redraw
// path re-run its own reset (x/y counters + failed-tile retries) and repopulate,
// so we only touch what we must here.
void MapPanel::releaseTiles(void)
{
    tiles.clear();          // reclaim the memory immediately
    needsRedraw = true;     // full rebuild next time this map draws
    redrawCompleted = false;
}
